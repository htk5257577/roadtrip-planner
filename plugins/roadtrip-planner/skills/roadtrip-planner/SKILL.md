...
`assets/roadtrip-planner-demo.html` is served by the local plugin server as one guided planning flow. Step one starts blank and collects departure, destination, ordered must-go places, departure/end dates, and departure time before any candidate generation. The fuzzy location search uses the bundled nationwide province/prefecture/county snapshot in `assets/locations.js`; custom scenic spots remain valid must-go places. It keeps the active page state in memory, exposes WebMCP tools when the host supports them, and queues explicit candidate/report requests for this conversation. Unlocated places must not receive invented map positions, mileage, or drive times. Page-only route previews remain estimates; the final report must distinguish them from facts verified by Codex and travel tools.
