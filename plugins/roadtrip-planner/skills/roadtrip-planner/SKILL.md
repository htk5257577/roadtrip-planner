...
9. Never embed personal API keys in a shareable skill or exported report. The page's real AMap preview uses a local 高德 Web Service key from the setup page or `AMAP_MAPS_API_KEY`. Without it, show the explicit missing-data state; do not substitute a decorative schematic or claim the map loaded. FlyAI uses its own optional key and CLI, independent of 高德. Neither is an OpenAI API key; this planner does not need one.
