...
To open the planner from another device on a trusted LAN, start it with `ROADTRIP_LAN=1 node <this-folder>/scripts/start.mjs`. The result includes `lanUrl` and a per-service `lanAccessCode`. Open `lanUrl` on the other device and enter `roadtrip` as the username and the access code as the password. The Codex bridge stays on `127.0.0.1`; remote devices cannot connect to its bridge endpoints or change provider keys. If the computer has multiple private IPv4 interfaces, set `ROADTRIP_LAN_IP` to the desired local address. The LAN page uses plain HTTP, so do not expose its port to the public internet. A running local-only service must be ended before enabling LAN access.
