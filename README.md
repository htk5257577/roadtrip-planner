...
需要从同一局域网的手机或电脑访问时，启动任务中说明“开启局域网访问”。服务仍保留 `127.0.0.1:4317` 给 Codex 使用，另在本机的局域网 IPv4 地址监听同一端口。启动结果会给出局域网地址和一次性的访问码；远端浏览器输入用户名 `roadtrip` 和该访问码。服务密钥只能在运行 Codex 的电脑上配置。局域网连接使用普通 HTTP，请只在可信网络使用，不要做公网端口转发。已运行的本机模式服务需先在原会话结束，再以局域网模式重新启动。
